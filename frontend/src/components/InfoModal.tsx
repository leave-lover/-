...
const InfoModal: React.FC<InfoModalProps> = ({ isOpen, onClose, title, data }) => {
  ...
  const formatValue = (key: string, value: any): string => {
    if (typeof value === "number") {
      // 对于电压、功率等数值，保留适当的小数位
      if (key.includes("vm") || key.includes("va") || key.includes("pd") || key.includes("qd") || key.includes("pg") || key.includes("qg")) {
        return value.toFixed(4);
      }
      // 对于容量、负载等数值，保留两位小数
      if (key.includes("capacity") || key.includes("load") || key.includes("value")) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          <button className="modal-confirm" onClick={onClose}>确定</button>
        </div>
      </div>
    </div>
  );
