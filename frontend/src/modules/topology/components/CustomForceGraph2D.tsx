...
>(
  (
    ...
    // 定义节点数据应该包含的字段
    const getNodeDataFields = (node: Node): Record<string, any> => {
      const allowedFields = [
        "id",
        "name",
        "type",
    ...
    // 定义连线数据应该包含的字段
    const getLinkDataFields = (link: Link): Record<string, any> => {
      const allowedFields = [
        "source",
        "target",
        "type",
      ...
      allowedFields.forEach((field) => {
        if (field in link) {
          // 特殊处理source和target字段，确保它们是字符串而不是对象
          if (field === "source" || field === "target") {
            if (
              typeof link[field] === "object" &&
              link[field] !== null &&
              "id" in link[field]
            ) {
              filteredData[field] = link[field].id;
            } else {
              filteredData[field] = link[field];
            }
          } else {
            filteredData[field] = link[field];
          }
    ...
    const handleNodeClick = (node: Node) => {
      ...
      setModalData(getNodeDataFields(node));
      setIsModalOpen(true);
    ...
    const handleLinkClick = (link: Link) => {
      ...
      setModalData(getLinkDataFields(link));
      setIsModalOpen(true);
