...
def main():
    ...
    for bilevel_iter in range(max_bilevel_iter):
        ...
        # 打印敏感性系数，用于调试
        print(f"  敏感性系数 (前5个电力节点): {sensitivity_d[:5]}")
        print(f"  敏感性系数 (前3个天然气节点): {sensitivity_l[:3]}")
        ...
        if not success_upper:
            ...
            # 保持之前的值，但打印警告
            print(f"  警告：上层问题求解失败，保持上层目标值为 {current_upper_objective:.6f}")
        else:
            # 打印新的上层目标值，用于调试
            print(f"  新的上层目标值: {upper_obj_val:.6f}")
    ...
    output_data = {
        ...
        "best_attack_cost": best_attack_strategy.get('lower_objective', float('inf')) if best_attack_strategy else float('inf'),
        "upper_objective": obj_upper_history,
        "lower_cost": obj_lower_history,
        "best_strategy": {
            "electric_attack_nodes": (best_elec_nodes + 1).tolist() if best_attack_strategy and len(best_elec_nodes) > 0 else [],
            "gas_attack_nodes": (best_gas_nodes + 1).tolist() if best_attack_strategy and len(best_gas_nodes) > 0 else [],
            "electric_attack_intensity": {
                str(node + 1): float(best_attack_strategy['z_a_d'][node] * baseMVA)
                for node in best_elec_nodes
            } if best_attack_strategy and len(best_elec_nodes) > 0 else {},
            "gas_attack_intensity": {
                str(node + 1): float(best_attack_strategy['z_a_l'][node])
                for node in best_gas_nodes
            } if best_attack_strategy and len(best_gas_nodes) > 0 else {}
        },
        "iteration_count": len(obj_upper_history)
    }
