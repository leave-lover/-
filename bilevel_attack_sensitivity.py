...
def main(tau_d_value=None):
    ...
    print("参数敏感性分析版本")
    if tau_d_value is not None:
        print(f"参数tauD值: {tau_d_value}")
    print("=" * 60)
    ...
    # 使用传入的tau_d参数，如果没有传入则从环境变量读取，最后使用默认值
    import os
    if tau_d_value is not None:
        tau_d = tau_d_value
    else:
...
def main_with_json_output(tau_d_value=None):
    """主函数，返回JSON格式的结果"""
    best_strategy, upper_hist, lower_hist, penalty_hist = main(tau_d_value)
    
    # 构建JSON格式的输出
    result = {
        "best_attack_cost": 0,
        "last_iteration_attack_objective": 0,
        "upper_objective": [],
        "lower_cost": [],
        "baseline_cost": 0,
        "best_strategy": {}
    ...
    # 总是提取最终成本和攻击目标值，不管best_strategy是否存在
    # 提取最终成本（最后一个迭代的成本）
    if len(lower_hist) > 0:
        result["best_attack_cost"] = lower_hist[-1]
    
    # 提取最后一次迭代的攻击目标值
    if len(upper_hist) > 0:
        result["last_iteration_attack_objective"] = upper_hist[-1]
    
    # 提取上层目标值历史
    result["upper_objective"] = upper_hist
    
    # 提取下层成本历史
    result["lower_cost"] = lower_hist
...
if __name__ == "__main__":
    # 从环境变量读取参数值
    import os
    tau_d_value = os.environ.get('TAU_D')
    if tau_d_value is not None:
        tau_d_value = float(tau_d_value)
    
    # 运行主函数并输出JSON结果
    result = main_with_json_output(tau_d_value)